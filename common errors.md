...
Ex: The connecton to the server 10.10.0.0:6443 refused
 Solution:
  ...
  Verify the logs by running crictl ps -a | grep api it show the exited container and run crictl logs kube-apiserver to verify the logs 
  and also cd /var/logs/containers/
   crictl ps 
   cirtl ps | grep api
   go to /etc/kubernetes/manifests/
   verify the kube-api server yaml file and validate the entries are correct in the yaml file 
   and verify the kubectl get nodes 
